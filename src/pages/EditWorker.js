...
import { FiArrowLeft, FiCamera, FiX, FiMapPin, FiRotateCcw, FiImage, FiFileText, FiUpload } from "react-icons/fi";
import defaultAvatar from "../assets/images/default_profile.png";

const suggestedTags = ["mechanic", "engineer", "tutor", "electrician", "driver", "teacher"];
...
export default function EditWorker() {
            ...
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    const workerData = {
                        ...
                        tags: data.tags || [],
                        profilePhotoUrl: data.avatarUrl || null,
                        ...
                        files: (data.files || []).map((url, i) => {
                            ...
                            return { url, name, isExisting: true };
                        })
                    };
                    ...
                    setProfilePhotoUrl(workerData.profilePhotoUrl);
    ...
    const handleReset = () => {
        ...
        setProfilePhotoUrl(originalData.profilePhotoUrl);
    ...
    const onSubmit = async () => {
        ...
        if (!title || !locationArea || !locationCity || !pincode) {
        ...
        try {
            ...
            if (profilePhotoFile) {
                finalProfilePhotoUrl = await uploadFileToCloudinary(profilePhotoFile);
            }

            const finalFileUrls = [];
            for (const f of uploadedFiles) {
                if (f.isExisting) {
                    finalFileUrls.push(f.url);
                } else if (f.file) {
                    const url = await uploadFileToCloudinary(f.file);
            ...
            await updateDoc(doc(db, "workers", id), {
                avatarUrl: finalProfilePhotoUrl,
                title: title.trim(),
            ...
            setError("Failed to update worker profile. Try again.");
            setSubmitting(false);
            setUploading(false);
            setSaveConfirm(false);
        }
        ...
        <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-pink-50">
            <div className="max-w-2xl mx-auto px-4 py-6 pb-24">
                ...
                <div className="mb-6">
                    ...
                    <p className="text-gray-600">Update worker profile details</p>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                        ...
                        <div className="space-y-3">
                            <input type="text" placeholder="Area" className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50" value={locationArea} onChange={(e) => setLocationArea(e.target.value)} required />
                            <input type="text" placeholder="Landmark (optional)" className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50" value={locationLandmark} onChange={(e) => setLocationLandmark(e.target.value)} />
                            <div className="grid grid-cols-2 gap-3">
                                <input type="text" placeholder="City" className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50" value={locationCity} onChange={(e) => setLocationCity(e.target.value)} required />
                                <input type="text" placeholder="Pincode" className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50" value={pincode} onChange={(e) => setPincode(e.target.value)} required />
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <input type="text" placeholder="Latitude" className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50" value={latitude} onChange={(e) => setLatitude(e.target.value)} required />
                                <input type="text" placeholder="Longitude" className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50" value={longitude} onChange={(e) => setLongitude(e.target.value)} required />
                            </div>
                        </div>
                    </div>
